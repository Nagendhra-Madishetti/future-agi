...
The default Compose stack runs frontend, backend, worker, agentcc-gateway, serving, code-executor, postgres, clickhouse, redis, minio, temporal, and fi-collector from published images. Redis fronts the cache, Channels layer, and Celery broker — no separate RabbitMQ.
...
| `redis` | Cache, rate limits, Celery broker, Django Channels layer, WebSocket pub/sub, distributed locks. (Replaces the separate RabbitMQ broker used in older releases.) |
