...
def stamp_evaluation_axes(evaluation: Any) -> None:
    ...
    try:
        if evaluation.eval_template_id is not None:
            ...
            multi_choice = bool(evaluation.eval_template.multi_choice)
    ...
    try:
        axes = resolve_eval_axes(evaluation.value, config_output, multi_choice)
    except Exception:
        logger.warning(
    ...
    if evaluation.output_bool is None and axes["output_pass"] is not None:
        evaluation.output_bool = axes["output_pass"]
    if evaluation.output_float is None and axes["output_score"] is not None:
        evaluation.output_float = axes["output_score"]
    if evaluation.output_str_list is None and axes["output_choices"] is not None:
        evaluation.output_str_list = axes["output_choices"]
